...
public class PaintJob {
	public static int getBucketCount(double width, double heigth, double areaPerBucket, int extraBucket) {
		if (width <=0 || heigth<=0 ||  areaPerBucket<=0 || extraBucket<0) {
			return -1;
		}else {
			int bucketsRequired = getBucketCount(width, heigth, areaPerBucket);
			bucketsRequired = bucketsRequired - extraBucket;
	...
	public static int getBucketCount(double width, double heigth, double areaPerBucket) {
		if (width <=0 || heigth<=0 ||  areaPerBucket<=0) {
			...
			int bucketsRequired = getBucketCount(area, areaPerBucket);
			return bucketsRequired;
		}
	}
